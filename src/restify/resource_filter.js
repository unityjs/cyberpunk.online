...
function Filter(opts) {
  ...
  this.filteredKeys = isPlainObject(opts.filteredKeys)
    ? { private: opts.filteredKeys.private || [], protected: opts.filteredKeys.protected || [] }
    : { private: [], protected: [] }
...
Filter.prototype.isExcluded = function (field, opts) {
  if (!field)
    return false
  opts = defaults(opts, { access: 'public', excludedMap: {}, filteredKeys: this.filteredKeys, modelName: this.model.modelName })
...
Filter.prototype.filterItem = function (item, excluded) {
  if (Array.isArray(item)) return item.map((i) => this.filterItem(i, excluded))
  if (item && excluded) {
    if (typeof item.toObject === 'function') item = item.toObject()
    for (let i = 0, length = excluded.length; i < length; i++) {
      if (excluded[i].indexOf('.') > 0) weedout(item, excluded[i])
      else delete item[excluded[i]]
    }
  }
  return item
}
...
Filter.prototype.filterPopulatedItem = function (item, opts) {
  if (Array.isArray(item)) return item.map((i) => this.filterPopulatedItem(i, opts))

  for (let i = 0; i < opts.populate.length; i++) {
    if (!opts.populate[i].path) continue
    ...
    if (has(item, opts.populate[i].path)) this.filterItem(get(item, opts.populate[i].path), excluded)
    else {
      ...
      if (has(item, pathToArray)) {
        ...
        if (Array.isArray(array)) this.filterItem(array.map((element) => get(element, pathToObject)), excluded)
      }
    }
  }
  return item
...
Filter.prototype.filterObject = function (resource, opts) {
  ...
  if (opts.populate) this.filterPopulatedItem(filtered, opts)
  return filtered
}
