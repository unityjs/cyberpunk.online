...
module.exports = function (options) {
  function jsonQueryParser(key, value) {
    if (key === '$regex' && !options.allowRegex) return undefined
    if (Array.isArray(value) && key[0] !== '$') return { $in: value }
    return value
  }
  ...
  function parseQueryOptions(queryOptions) {
    if (queryOptions.select && typeof queryOptions.select === 'string') {
      ...
      for (let i = 0, length = select.length; i < length; i++) {
        if (select[i][0] === '-') queryOptions.select[select[i].substring(1)] = 0
        else queryOptions.select[select[i]] = 1
      }
    }
    ...
    if (queryOptions.populate) {
      if (typeof queryOptions.populate === 'string') {
        ...
        for (let i = 0, length = populate.length; i < length; i++) {
          queryOptions.populate.push({ path: populate[i], })
          for (const key in queryOptions.select) {
            if (key.indexOf(populate[i] + '.') === 0) {
              if (queryOptions.populate[i].select) queryOptions.populate[i].select += ' '
              else queryOptions.populate[i].select = ''
              if (queryOptions.select[key] === 0) queryOptions.populate[i].select += '-'
      ...
      } else if (!Array.isArray(queryOptions.populate)) queryOptions.populate = [queryOptions.populate]
    }
    return queryOptions
  }
  ...
  return function (req, res, next) {
    ...
    for (const key in req.query) {
      ...
      if (key === 'query')
        try { query[key] = JSON.parse(req.query[key], jsonQueryParser) } catch (e) { return next(new Error(`invalid_json_${key}`)) }
      else if (key === 'populate' || key === 'select' || key === 'sort')
        try { query[key] = JSON.parse(req.query[key]) } catch (e) { query[key] = req.query[key] }
      else if (key === 'limit' || key === 'skip') {
        query[key] = parseInt(req.query[key], 10)
        if (`${query[key]}` !== `${req.query[key]}`) return next(new Error(`invalid_${key}_value`))
